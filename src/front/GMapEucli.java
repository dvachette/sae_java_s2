...
public class GMapEucli extends JComponent {
    ...
    public void paintParcours(Graphics gr) {
        ...
        PointEuclidien origin = parcours.getPath().getFirst();
        PointEuclidien next = null;
        for (int i = 1; i < parcours.getPath().size(); i++) {
            next = parcours.getPath().get(i);
            paintDistance2(gr, origin.getId(), next.getId());
            origin = next;
        }
        next = parcours.getPath().getFirst();
        paintDistance2(gr, origin.getId(), next.getId());
    ...
    public void paintDistance2(Graphics gr, int idStart, int idEnd) {
        ...
        for (PointEuclidien p : listePointsConvertis) {
            if (p.getId() == idStart) {
                pStart = p;
            }
            if (p.getId() == idEnd) {
